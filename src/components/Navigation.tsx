...
import { LayoutGrid, Calendar, User, Shield, Mic, MessageSquare, HelpCircle } from 'lucide-react';
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 pb-safe md:relative md:border-t-0 md:bg-transparent z-50">
      <div className="flex justify-around items-center p-2">
        ...
        <button
          onClick={() => onTabChange(AppView.SUPPORT)}
          className={`p-3 rounded-2xl flex flex-col items-center gap-1 transition-all ${activeTab === AppView.SUPPORT ? 'text-teal-600 dark:text-teal-400' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
        >
          <HelpCircle size={24} strokeWidth={activeTab === AppView.SUPPORT ? 2.5 : 2} />
